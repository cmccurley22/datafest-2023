...
df_attorney_data <-
  subset(df_attorney_data, select = -c(Id.x, Id.y))
...
df_time_per_state <- df_attorney_data %>% 
  group_by(StateAbbr) %>% 
  summarise(total_hours = sum(Hours), average_hours = mean(Hours))

df_time_per_state %>% 
ggplot() +
  geom_point(aes(StateAbbr, average_hours))
